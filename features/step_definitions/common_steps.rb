...
And(/^I enable Suse container repository, but not for SLES 11 systems$/) do
  ...
  # there is no Docker Repos for SLES 11 systems.
  _out, code = $minion.run('pidof systemd', false)
  # only for sle12 and major systems
  apply_container_repos if code.zero?
end

And(/^I disable Suse container repository, but not for SLES 11 systems$/) do
...
And(/^I enable SLES pool and update repository on "([^"]*)", but not for SLES 11 systems$/) do |target|
