...
Feature: XML-RPC image namespace for containers
  ...
  Scenario: Apply the highstate to ensure container build host is ready
    ...
    When I enable Suse container repos, but not for SLES 11 systems
    And I enable sles pool and update repo on "sle-minion", but not for SLES 11 systems
  ...
  Scenario: Cleanup: remove custom system info key
    ...
    And I disable Suse container repos, but not for SLES 11 systems
