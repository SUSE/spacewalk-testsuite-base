...
Feature: Be able to build container images
  ...
  Given I am authorized as "admin" with password "admin"
  And I schedule the build of image "suse_key" with version "Latest_key-activation1" via XML-RPC call 
  And I schedule the build of image "suse_simply" with version "Latest_simply" via XML-RPC call 
  And I verify that all "5" container images were built correctly in the gui

  Scenario: Delete image via XML-RPC calls
  Given I am authorized as "admin" with password "admin"
  And I delete the image "suse_key" with version "Latest_key-activation1" via XML-RPC call
  And I delete the image "suse_simply" with version "Latest_simply" via XML-RPC call
  And The image "suse_simply" with version "Latest_key-activation1" doesn't exist via XML-RPC call
  And The image "suse_simply" with version "Latest_simply" doesn't exist via XML-RPC call
  And I schedule the build of image "suse_simply" with version "Latest_simply" via XML-RPC call
  And I schedule the build of image "suse_key" with version "Latest_key-activation1" via XML-RPC call 
